...
4. Commit and push the release candidate to `main`; require fresh hosted Windows/Node 24, Ubuntu/Node 20, and Pages checks for that exact commit before tagging. The full local release gate covers stress, audit, and package integrity before tagging; the tag-triggered Release workflow independently repeats that complete gate on the released commit. Do not restore the former duplicate per-push Release Integrity job unless it protects a new, distinct risk.
