...
- Windows smoke tests that spawn child processes and then remove temporary trees must use bounded `fs.rm` retries for transient `EBUSY`/`EPERM`/`ENOTEMPTY` cleanup races after real teardown completion; prefer condition-based teardown evidence over arbitrary sleeps. Always remove test-owned stress fixture roots in `finally` and surface cleanup errors; instrument process teardown and job fixture setup separately when diagnosing hosted-only stalls. Do not delete pre-existing temporary folders without verifying ownership and authorization.
- Windows exclusive-lock test helpers must acknowledge lock acquisition before starting a parent-timed hold; wait for an explicit parent release to avoid expiring the lock before a loaded CI runner begins its retry probe. Bound readiness and teardown, and retain a real exclusive-lock regression for both stores.
- Windows user-state stores that atomically replace an existing file must tolerate short-lived `EPERM`/`EACCES`/`EBUSY` rename locks with bounded retry/backoff and retain temp-file cleanup on terminal failure. The Job and Goal stores use a bounded roughly five-second retry window because hosted runners can hold locks beyond the earlier roughly one-second window; keep longer-than-old-window exclusive-lock regressions in both existing smoke tests. Non-Windows and unrelated errors must fail promptly.
...
4. Commit and push the release candidate to `main`; require fresh hosted Windows/Node 24, Ubuntu 24.04/Node 20, and Pages checks for that exact commit before tagging. Pin Ubuntu runner versions in both push CI and tag-triggered Release instead of implicitly accepting runner-image migrations; adopt new distributions through explicit compatibility verification. The full local release gate covers stress, audit, and package integrity before tagging; the tag-triggered Release workflow independently repeats that complete gate on the released commit. Do not restore the former duplicate per-push Release Integrity job unless it protects a new, distinct risk.
