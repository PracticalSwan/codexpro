...
- HTTP/MCP/admin durability or continuity tests that persist workspace-owned state must carry the server `workspace_id` when applicable and persist the server/canonical workspace root rather than an input/temp alias; session-local workspace selection and Windows 8.3 path aliases are not durable identity.
