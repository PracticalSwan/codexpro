...
6. If runtime stop/restart was explicitly authorized, stop only the owned CodexPro process tree, install the exact released artifact into the intended global Node environment, verify critical source/install hashes plus protected profile/hook settings, then restart with the same effective roots/settings and verify live health/version/tunnel arguments. Never touch unrelated tunnel clients.
