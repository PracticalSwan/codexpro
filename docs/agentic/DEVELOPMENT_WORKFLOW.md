...
- Cross-session HTTP/MCP continuity tests must carry the original `workspace_id` explicitly and persist the server-returned canonical workspace root rather than an input/temp alias; session-local workspace selection and Windows 8.3 path aliases are not durable identity.
