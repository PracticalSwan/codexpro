...
- Structured detached workers that depend on parent-side PID/start-identity attestation must keep the nonce/start-key-gated pre-claim window longer than the parent's bounded identity-probe budget; a live process may receive only a bounded startup-attestation grace when identity probing is temporarily unavailable, while dead PIDs or concrete identity mismatches still fail immediately. The worker should prove ownership with the launch nonce and parent-attested start key rather than spawning a second OS identity probe; any cancellation or signal must still re-read the current OS start identity and match PID + nonce + start key. Do not weaken PID-reuse checks to solve startup latency.
- HTTP/MCP durability or continuity tests that persist workspace-owned state must carry the server `workspace_id` explicitly and persist the server-returned canonical workspace root rather than an input/temp alias; session-local workspace selection and Windows 8.3 path aliases are not durable identity.
...
Telegram continuation is optional/default-off and is not requested or started unless task continuation is already enabled and Telegram is explicitly enabled. It uses a dedicated private bot plus outbound long polling. Plan 37's deterministic implementation and real Bot API doctor/test boundary are complete on the cumulative branch; final Telegram-button-to-managed-browser live acceptance remains a human/runtime gate before Plan 37 can be marked Verified. If bot creation/token configuration is required, STOP and report `WAITING_FOR_TELEGRAM_BOT_TOKEN`; instruct the user to use `@BotFather` and CodexPro's masked local token-save command, never chat. Resume only after the user sends `continue`. Then generate the one-time private-bot pairing link/code, STOP as `WAITING_FOR_TELEGRAM_PAIR`, and resume only after the user presses Start/completes pairing and sends `continue`.
