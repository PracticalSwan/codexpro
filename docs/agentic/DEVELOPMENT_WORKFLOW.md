...
- Task-aware continuation is optional/default-off. Plans 29–35 and 37 are verified; Plan 36 remains the final fresh-session acceptance gate. When enabled, continuation derives interruption timing from current runtime status (`runtimeGenerationId`, deadline mode/value, transport readiness), never from saved/default settings; Unlimited/observe disables timeout inference entirely. After Plan 36 closes, treat continuation as feature-frozen except for evidence-backed defects.
- Runtime/tunnel loss, restart, browser reconnect after a long gap, manual user message/Stop action, task completion/cancel, auth loss, wrong/stale chat route, streaming, or generic platform busy/error/retry/unknown UI suppress continuation; the continuation layer itself must not auto-Retry, switch models, or start/restart CodexPro/tunnels. Independent launcher-level tunnel-client supervision may recover its own failed child, but continuation must observe the runtime `transportState` transition and cannot dispatch through the outage or treat recovery as authorization.
...
- With `continuationEnabled=true`, arm only substantial work likely to span calls; checkpoint after each coherent material milestone and before starting a long/high-variance phase rather than waiting for the deadline or host cutoff, keep actual execution in canonical `proc_*`/`job_*`/`goal_*`/`batch_*` state, request continuation before a truthful semantic yield, and keep every send user-gated. With continuation disabled, no semantic continuation checkpoint is created automatically. On a resumed turn call `continuation_status` first, recover current runtime plus canonical durable state, and avoid redoing verified work.
- When `continuation_status` reports `manualTurnPending`, reconcile exactly once with `continuation_reconcile`: `resume` for ordinary continuation, `redirect` for changed priorities inside the same goal, `supersede` for a materially new task, or `cancel` for explicit stop/cancel; ambiguity remains paused. With `continuationEnabled=false`, do not arm continuation and do not initiate browser or Telegram setup.

## Browser authentication stop rule
- For Plan 36 acceptance or any later authorized continuation live verification, any ChatGPT/provider sign-in, CAPTCHA, passkey, 2FA, email confirmation, or similar security verification is a mandatory human boundary.
...
Telegram continuation is optional/default-off and is not requested or started unless task continuation is already enabled and Telegram is explicitly enabled. Plan 37 is verified at its intended private-bot/outbound-long-polling authorization scope. Plan 36 still requires one installed-runtime fresh-session acceptance of the overall continuation system. If bot creation/token configuration is ever required for an explicitly authorized test, STOP and report `WAITING_FOR_TELEGRAM_BOT_TOKEN`; instruct the user to use `@BotFather` and CodexPro's masked local token-save command, never chat. Resume only after the user sends `continue`. Then generate the one-time private-bot pairing link/code, STOP as `WAITING_FOR_TELEGRAM_PAIR`, and resume only after the user presses Start/completes pairing and sends `continue`.
...
Plan 36 final acceptance uses a new ChatGPT conversation with installed CodexPro Full to run the documented disposable acceptance matrix and emit a sanitized Markdown report. The user copies that report back for maintenance review; the test session does not mutate CodexPro source unless separately authorized. Product defaults remain continuation=false and Telegram=false. Do not enable either profile setting unless the operator explicitly requests that live setup.
