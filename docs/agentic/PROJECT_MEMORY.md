...
- On 2026-09-08, Plans 22–28 were added as **planning-only** deadline-resilience work. The design fixes the future synchronous MCP call budget at exactly `1,200,000` ms (20 minutes) while explicitly preserving full task scope, acceptance criteria, review depth, and required verification across continuation calls. Planned roles are synchronous deadline propagation, durable structured `job_*` state for long verification, existing `proc_*` for long shell work, existing Durable Goals for multi-stage isolated work, resumable `batch_*` cursors for expensive in-process work, and bounded diagnostics. No Plans 22–28 runtime behavior is implemented yet.
