...
- Public release `v0.33.4` supersedes `v0.33.3`, with archive and SHA-256 sidecar hosted on GitHub Releases and current installation instructions live on Pages. A public-artifact disposable installation verified release identity. The global `0.33.2` installation must not be replaced yet: the repository workspace reports stopped, but an operator-owned `0.33.2` runtime serving another workspace remains active through the OpenAI tunnel. Preserve that runtime until the operator explicitly approves stopping its exact owned process tree, then reinstall the verified release artifact, compare all seven profile and user-preference hashes, and leave it stopped. At the last check, all seven profiles and the future-profile user preference enabled Local Service Probe. npm registry publication remains unavailable without independently verified authentication or trusted publishing.
