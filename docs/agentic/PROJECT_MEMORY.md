...
- Current public stable release: **v0.33.3**, tag commit `fb20029d74b34133c577d00c6c8f78ea61a3bee7`; published GitHub tarball SHA-256 `5eda652c50c53939af4e03efe962b6ce9ef92f3958eb060de228a7e9604f0a3f` independently verified against the release sidecar and via disposable installation. GitHub Releases are canonical; npm-registry publication remains a separately authenticated channel.
...
- The 2026-09-23 (Bangkok time) v0.33.3 patch passed the complete local release gate, hosted Windows/Node 24, Ubuntu/Node 20, Release Integrity, Pages, and tag-triggered GitHub Release workflow. The published archive digest and sidecar match; its CLI build identity was independently verified by disposable install. The patch fixes `codexpro doctor` incorrectly treating the same workspace's identity-verified running runtime as an unrelated occupied-port blocker; stale and unrelated collisions remain blocking. At the latest 2026-09-23 inspection, the operator's previously running v0.33.2 instance was already stopped; no agent stopped or restarted it. A released-artifact upgrade must recheck that it remains stopped before replacing the global installation. The preceding v0.33.2 patch corrected Artifact Export registration diagnostics and provided the per-OS-user future-profile Local Service Probe opt-in while preserving the package-wide default-off boundary and the operator's previously backed-up profile settings.
- After the v0.33.3 documentation closeout, the same source passed release-tag CI but the later `main` Windows/Node 24 smoke intermittently failed with `EPERM` during structured Job atomic record replacement. A local exclusive-lock regression reproduced the previous short retry window's failure; the v0.33.4 patch candidate extends bounded transient Windows rename retries for both Job and Goal stores and adds longer-lock coverage. The full local v0.33.4 release gate passed (release guard, TypeScript build, smoke, stress, dependency audit with zero reported vulnerabilities, and package validation); fresh hosted Windows/Node 24, Ubuntu/Node 20, Release Integrity, and Pages checks remain required before publication. No worker-ownership, authorization, or runtime-lifecycle behavior changes.
...
- Public release `v0.33.3` supersedes `v0.33.2`; its archive and SHA-256 sidecar are hosted on GitHub Releases, with canonical installation instructions on GitHub Pages. npm-registry publication of `codexpro-full` is unavailable because this machine has no existing npm authentication or configured trusted-publishing path.
