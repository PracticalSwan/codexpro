...
- On 2026-09-08, Plan 22 implemented baseline tool-time awareness: normal bounded mode defaults to exactly `1,200,000` ms (20 minutes), finite settings are 5–60 minutes, and explicit Unlimited/observe mode preserves a finite reference while disabling only CodexPro's cooperative cutoff. Runtime/profile state, launcher/settings, authenticated admin editing, `server_config`, and per-MCP-call `AsyncLocalStorage` deadline context now share this contract; saved web/profile changes remain next-run only. The selected value is transport-only and may never reduce task scope, acceptance criteria, review depth, required verification, or safety checks. Plans 25–28 remain responsible for async verification, execution routing, resumable `batch_*`, and final diagnostics/tool-time probing.
...
- On 2026-09-08, Plans 29–37 were added and edge-case reviewed as **planning-only** task-aware continuation work. Continuation is a separate optional subsystem disabled by default; Telegram is also disabled by default and setup is skipped unless continuation + Telegram are explicitly enabled. The current operator has requested both enabled later for their own local profile during authorized implementation/setup, without changing product defaults. The design uses durable revisioned semantic continuation state, a separate least-privilege loopback browser companion, a dedicated Chrome/Edge profile with manual sign-in, stable explicit chat binding, current-runtime deadline/generation/transport synchronization, conservative restart/sleep/disconnect handling, terminal/user-pause precedence, and an explicit **Continue task** click for every dispatch. Generic ChatGPT busy/error/retry/unknown UI, stopped transport, auth loss, or ambiguity suppress continuation; no auto-Retry/model switch/tunnel restart is planned. A manual user prompt atomically invalidates stale browser/Telegram actions without storing text, then the semantic controller reconciles it as resume, redirect, supersede-new-task, or cancel; ambiguity remains paused. Implementation/live QA must STOP for any ChatGPT/provider authentication/security verification and resume only after the user authenticates directly in the browser and sends `continue`. The companion must not scrape ChatGPT output or attempt to bypass host tool/session restrictions. No Plans 29–37 runtime behavior is implemented yet.
