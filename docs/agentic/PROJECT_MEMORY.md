...
Last verified: 2026-09-24 (Asia/Bangkok)
...
- Current public stable release: **v0.33.4**, tag commit `e73c91132c5a79bb39b3f2590e3160c3e556a877`; public tarball SHA-256 `841c4891ef140d478acbc8d5a90491e44dfe7dbcee4610d7bdc35d80d2a22bb1` matches its sidecar and a disposable install verified version, release channel, and exact build revision. GitHub Releases are canonical; npm-registry publication remains a separately authenticated channel.
...
- The v0.33.3 patch fixed `codexpro doctor` misidentifying a same-workspace, identity-verified active runtime as an unrelated port collision; stale and unrelated collisions remain blocking. Its release gate and public artifact were independently verified. The preceding v0.33.2 patch corrected Artifact Export diagnostics and provided the per-OS-user future-profile Local Service Probe opt-in without changing the package-wide default-off boundary.
- The v0.33.4 release extends bounded Windows atomic-replacement retries for Job and Durable Goal stores to approximately five seconds and adds exclusive-lock regressions reproducing the prior `EPERM` failure. The complete local release gate passed (build, smoke, stress, zero-vulnerability audit, package validation). The redundant per-push Release Integrity job was removed; fresh Windows/Node 24 and Ubuntu/Node 20 smoke plus Pages passed on its tagged commit, and the tag-triggered Release workflow independently passed its full gate and published the verified tarball and checksum. Ownership, authorization, and runtime-lifecycle protections remain unchanged.
- A post-release documentation-only `main` push exposed a separate Windows smoke-fixture failure: the PowerShell exclusive-lock child did not acknowledge readiness within its old timeout. Subsequent test-only `main` changes share a parent-held lock fixture with explicit readiness/release, prefer PowerShell 7 on hosted Windows, and tolerate the historical `slow.pid` startup race without skipping teardown checks for children that actually started. The complete local release gate passed again on Windows. Push-triggered and independently dispatched Windows/Node 24 and Ubuntu/Node 20 CI both passed on the exact post-tag `main` commit `51788396a0a295cf5b29c15fe507bf0ed795ae75`; Pages passed on that commit too. No runtime source or published v0.33.4 artifact was changed by these test fixes.
...
- Public release `v0.33.4` supersedes `v0.33.3`, with archive and SHA-256 sidecar hosted on GitHub Releases and current installation instructions live on Pages. A public-artifact disposable installation verified release identity. After the operator stopped the CSX4201 runtime, the SHA-256-verified public v0.33.4 GitHub Release archive was installed globally into the active Windows Node environment. The installed CLI/build identity matches the tagged release and all 306 packaged files matched archive contents byte-for-byte. All seven saved profile hashes and the per-user preference hash were unchanged, with Local Service Probe still enabled for every existing profile and future profiles. CodexPro was left stopped after installation; the operator subsequently started the CSX4201 runtime manually. Agents must not start or restart it. npm registry publication remains unavailable without independently verified authentication or trusted publishing.
- The operator-started CSX4201 runtime was verified running the exact v0.33.4 release with a ready OpenAI tunnel client (`/readyz` HTTP 200); unauthenticated local HTTP requests correctly return 401. Direct authenticated MCP requests through the user-connected tunnel were not available to this maintenance session and remain unverified.
- Shipped v0.33.4 repository inspection included 370 Git-ignored CSX4201 output files (403 total), including dependency metadata promoted as an important file. The post-release source fix now uses Git's tracked plus non-ignored untracked inventory, retaining force-tracked ignored paths, guarded path/text checks, and non-Git discovery. A failing-before/passing-after CLI regression and real CSX4201 rerun verified 25 files and zero ignored outputs. The complete local release gate, including stress, zero-vulnerability audit, and package integrity, passed. The running v0.33.4 instance remains unchanged; a newly authorized release and specific runtime stop are required to roll out the fix globally.
- To avoid GitHub's announced Ubuntu runner-label migration, `main` now pins Ubuntu 24.04 for its Node 20 smoke matrix and tag-triggered release workflow. Exact-commit Ubuntu/Node 20 and Windows/Node 24 hosted CI plus Pages passed after the pin; the updated tag-triggered release workflow has only been syntax-checked because no new release was authorized.
- The installed v0.33.4 CLI help still named the upstream `codexpro` npm package despite CodexPro Full's GitHub Release distribution. Post-release source now prints a version-matched CodexPro Full archive URL and the release index. The focused regression failed before and passed after the fix; the complete local release gate then passed, including the OpenAI-tunnel smoke, Windows stress, zero-vulnerability audit, and package validation. The user-owned v0.33.4 runtime was not replaced, stopped, or restarted. Changes on `main` require a separately authorized release and specific active-runtime stop to reach the global installation.
- Follow-up maintenance found a roughly five-minute gap between `process-smoke` completion and `jobs-smoke` startup on one successful hosted Windows run; standalone process smoke completed locally in about 1.3 seconds, so the hosted delay's root cause is not established. Diagnostic markers now bracket process fixture teardown and job fixture initialization. Windows Temp held 649 historical `codexpro-stress-*` fixture directories dating back to September 5; the reviewed stress cleanup changes use bounded `finally` teardown, and a focused local jobs/stress run left zero new stress fixture directories. Historical directories were not deleted without separate cleanup authorization.
