...
- On 2026-09-08, Plans 22–28 were added as **planning-only** deadline-resilience work and refined so tool-time awareness is enabled by default: normal bounded mode defaults to exactly `1,200,000` ms (20 minutes), finite settings are 5–60 minutes, and explicit Unlimited/observe mode is reserved for temporary harmless host-window discovery with a mutation-free probe. The selected value remains transport-only and may never reduce task scope, acceptance criteria, review depth, required verification, or safety checks. Planned roles are synchronous deadline propagation, durable structured `job_*` state for long verification, existing `proc_*` for long shell work, existing Durable Goals for multi-stage isolated work, resumable `batch_*` cursors for expensive in-process work, and bounded diagnostics. No Plans 22–28 runtime behavior is implemented yet.
