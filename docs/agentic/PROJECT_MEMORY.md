...
- On 2026-09-08, post-remediation live QA confirmed the previous policy, Context v2, CodeGraph, verification-path, activity-summary, and process-readiness fixes. Follow-up maintenance fixed the remaining activity-ledger self-recording defect and made Goal `isolation_active` reflect actual retained-worktree existence after projection cleanup. Fresh focused MCP/Goal regressions, full smoke, stress, audit (0 vulnerabilities), release-pack, and diff checks passed before integration.
