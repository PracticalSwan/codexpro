...
**Goal:** Make connector, tool-surface, operation, runtime, and backend failures diagnosable without logging prompts, source contents, tokens, or raw command output.

**Architecture:** Use one in-memory bounded telemetry registry with sanitized event summaries, optional durable counters only when explicitly enabled, and read-only projections for MCP and the local admin dashboard.
...
**Spec:** `docs/superpowers/specs/2026-09-05-observability-diagnostics-design.md`
...
**Plan dependencies:** 01, 03.

---

## File Structure
- Create: `src/telemetry.ts`
- Create: `src/diagnosticsOps.ts`
- Create: `scripts/diagnostics-smoke.mjs`
- Modify: `src/http.ts`
...
- Modify: `CHANGELOG.md`
- Test: `scripts/diagnostics-smoke.mjs`
...
- [ ] **Step 2: Add focused failing contract/regression cases for these behaviors:**
  - distinguish no request arrival from dispatch failure and response-write failure
  - configured/expected/registered tool sets are compared explicitly
  - telemetry counters are bounded and evict oldest detail records
  - diagnostics contain no auth tokens, prompts, file contents, absolute secret paths, or raw command output
  - admin dashboard works when telemetry persistence is disabled
...
- [ ] **Step 5: Commit only if the execution request authorizes commits.**
  ...
  Suggested message: `test: define observability diagnostics contracts`
...
- [ ] **Step 5: Commit if authorized.**
  Suggested message: `feat: add observability diagnostics core`
