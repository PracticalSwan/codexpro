# 0.31 Maintenance Baseline Implementation Plan
...
**Goal:** Create a clean, releasable 0.31.x baseline by reconciling the current integration branch with upstream main and the still-open maintenance PRs without duplicating fixes.

**Architecture:** Use a reconciliation matrix first, then land only independently verified maintenance changes. Treat release packaging as evidence, not as publication authorization.
...
**Spec:** `docs/superpowers/specs/2026-09-05-maintenance-baseline-design.md`
...
**Plan dependencies:** none beyond the refreshed current baseline.

---

## File Structure
- Create: `docs/agentic/maintenance-reconciliation.md`
- Modify: `CHANGELOG.md`
- Modify: `package.json`
...
- Test: `scripts/release-guard-smoke.mjs`
...
- [ ] **Step 2: Add focused failing contract/regression cases for these behaviors:**
  - every integration-only commit is mapped to an upstream PR/issue or documented local-only fix
  - duplicate PR implementations are not applied twice
  - package dry-run contains the intended runtime files and no agent-planning/private artifacts
  - npm audit --audit-level=high reports zero known high-severity vulnerabilities
...
- [ ] **Step 5: Commit only if the execution request authorizes commits.**
  ...
  Suggested message: `test: define maintenance baseline contracts`
...
- [ ] **Step 5: Commit if authorized.**
  Suggested message: `feat: add maintenance baseline core`
