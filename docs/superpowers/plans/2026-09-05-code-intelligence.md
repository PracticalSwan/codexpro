# Optional Code Intelligence Backends Implementation Plan
...
**Goal:** Add optional fuzzy/fast lexical search, CodeGraph structural analysis, LSP symbol intelligence, and dependency-aware context ranking while preserving built-in fallbacks.

**Architecture:** Formalize provider seams already present in analysis/providers.ts. Optional adapters advertise capabilities and availability; routing falls back to current ripgrep/Node/built-in analysis when external tools are missing, stale, or fail.
...
**Spec:** `docs/superpowers/specs/2026-09-05-code-intelligence-design.md`
...
**Plan dependencies:** 06, 07.

---

## File Structure
- Create: `src/searchBackends.ts`
- Create: `src/analysis/codegraphProvider.ts`
- Create: `src/analysis/lspProvider.ts`
- Create: `scripts/code-intelligence-smoke.mjs`
- Create: `docs/code-intelligence.md`
...
- [ ] **Step 2: Add focused failing contract/regression cases for these behaviors:**
  - fff/ffgrep absence falls back without failure
  - CodeGraph is never auto-installed and only bootstraps/syncs when explicitly enabled and CLI exists
  - stale CodeGraph index is detected before structural query
  - LSP adapter starts only explicitly configured/user-installed servers and terminates owned processes
  - provider paths are revalidated through PathGuard before results leave CodexPro
  - gather_context improves ranking from structural dependencies but remains usable with built-in analysis only
...
- [ ] **Step 3: Run only the focused new smoke script(s) and confirm the new assertions fail for the intended missing behavior, not because the harness/environment is broken.**
  ...
  Run: `node scripts/code-intelligence-smoke.mjs`.
...
- [ ] **Step 5: Commit only if the execution request authorizes commits.**
  ...
  Suggested message: `test: define code intelligence contracts`
...
- [ ] **Step 5: Commit if authorized.**
  Suggested message: `feat: add code intelligence core`
