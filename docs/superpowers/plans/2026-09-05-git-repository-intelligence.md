# Git and Repository Intelligence Implementation Plan
...
**Goal:** Provide structured Git history, package/monorepo topology, richer change impact, guarded Git mutations, and pre-commit safety checks through dedicated tools rather than ad-hoc Bash.

**Architecture:** Extend read-only Git behind gitOps.ts, keep write-side Git in a separate gitWriteOps module with explicit expected branch/HEAD/path constraints, and feed package topology into the existing analysis impact model.
...
**Spec:** `docs/superpowers/specs/2026-09-05-git-repository-intelligence-design.md`
...
**Plan dependencies:** 03, 05, 06.

---

## File Structure
- Create: `src/packageGraph.ts`
- Create: `src/gitWriteOps.ts`
- Create: `src/preflightOps.ts`
- Create: `scripts/git-intelligence-smoke.mjs`
- Create: `scripts/git-write-smoke.mjs`
- Modify: `src/gitOps.ts`
- Modify: `src/analysis/impact.ts`
- Modify: `src/analysis/types.ts`
- Modify: `src/server.ts`
- Modify: `src/config.ts`
- Modify: `CHANGELOG.md`
- Test: `scripts/git-intelligence-smoke.mjs`
- Test: `scripts/git-write-smoke.mjs`
...
- [ ] **Step 2: Add focused failing contract/regression cases for these behaviors:**
  - history tools remain read-only and path-scoped inside allowed roots
  - package graph discovers npm/Python/Cargo/Go manifests without executing package managers
  - impact analysis maps changed package to dependents and related tests
  - stage accepts explicit paths only and rejects blocked/unexpected files
  - commit requires expected HEAD and reports the exact staged set
  - push requires configured opt-in plus expected branch/HEAD and never force-pushes
  - preflight detects secret-like additions, conflict markers, unexpected binaries, oversized files, and blocked paths without scanning unrelated ignored content
...
- [ ] **Step 3: Run only the focused new smoke script(s) and confirm the new assertions fail for the intended missing behavior, not because the harness/environment is broken.**
  ...
  Run: `node scripts/git-intelligence-smoke.mjs`.
...
- [ ] **Step 5: Commit only if the execution request authorizes commits.**
  ...
  Suggested message: `test: define git repository intelligence contracts`
...
- [ ] **Step 5: Commit if authorized.**
  Suggested message: `feat: add git repository intelligence core`
