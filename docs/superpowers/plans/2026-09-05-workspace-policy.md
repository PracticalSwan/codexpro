...
**Goal:** Add an optional workspace-local policy that can tighten CodexPro limits and guidance without ever broadening the operator's global permissions.

**Architecture:** Parse one bounded policy document through a dedicated policy module, merge it monotonically with CodexProConfig, and expose the effective policy read-only through diagnostics.
...
**Spec:** `docs/superpowers/specs/2026-09-05-workspace-policy-design.md`
...
## File Structure
- Create: `src/policyOps.ts`
- Create: `scripts/policy-smoke.mjs`
- Create: `docs/workspace-policy.md`
...
- [ ] **Step 2: Add focused failing contract/regression cases for these behaviors:**
  - missing policy preserves current behavior
  - policy can add blocked globs and lower limits
  - policy cannot enable bash/write/tools disabled globally
  - symlinked or oversized policy files fail closed
  - invalid policy reports a bounded actionable error
...
- [ ] **Step 5: Commit only if the execution request authorizes commits.**
  ...
  Suggested message: `test: define workspace policy contracts`
...
- [ ] **Step 5: Commit if authorized.**
  Suggested message: `feat: add workspace policy core`
