# Workspace Processes and Verification Implementation Plan
...
**Goal:** Support workspace-owned long-running processes and structured build/test verification without turning CodexPro into a machine-wide process manager.

**Architecture:** A process registry owns only child processes launched through CodexPro and binds each to one workspace and operation receipt. A checks module detects existing project scripts, runs bounded verification, and parses known test output into a stable result model.
...
**Spec:** `docs/superpowers/specs/2026-09-05-process-verification-design.md`
...
**Plan dependencies:** 03, 04.

---

## File Structure
- Create: `src/processOps.ts`
- Create: `src/checksOps.ts`
- Create: `src/testResultOps.ts`
- Create: `scripts/process-smoke.mjs`
- Create: `scripts/checks-smoke.mjs`
...
- [ ] **Step 2: Add focused failing contract/regression cases for these behaviors:**
  - process cwd cannot escape selected workspace
  - only CodexPro-owned PIDs may be queried or stopped
  - output cursor is bounded and survives noisy child output
  - server shutdown terminates or explicitly detaches children according to policy
  - run_checks uses existing project scripts instead of synthesizing arbitrary commands
  - verify_changes recommends and runs the smallest relevant checks from change analysis
  - failed test parsing preserves raw bounded evidence and identifies test/file/line when available
...
- [ ] **Step 3: Run only the focused new smoke script(s) and confirm the new assertions fail for the intended missing behavior, not because the harness/environment is broken.**
  ...
  Run: `node scripts/process-smoke.mjs`.
...
- [ ] **Step 5: Commit only if the execution request authorizes commits.**
  ...
  Suggested message: `test: define process verification contracts`
...
- [ ] **Step 5: Commit if authorized.**
  Suggested message: `feat: add process verification core`
