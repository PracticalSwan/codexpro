...
> **Release status:** the latest tagged stable artifact is **0.33.6**. Patch 0.33.6 adds a bounded owner-verified MCP child-exit wait to `codexpro stop`, fixing a reproduced Windows launcher/server shutdown race without weakening PID/start-identity checks. Patch 0.33.5 respects Git ignore rules during workspace inspection, fixes canonical release install help, improves Windows stress-fixture cleanup and diagnostics, and pins Ubuntu CI/release runners to 24.04. Patch 0.33.4 extends bounded Windows atomic-replacement retries for structured Jobs and Durable Goals to handle longer transient file locks. Patch 0.33.3 fixes the `codexpro doctor` false port-conflict blocker for an identity-verified running runtime in the same workspace, while preserving unrelated collision failures. Patch 0.33.2 corrects Artifact Export registration diagnostics and adds a per-OS-user opt-in for future Local Service Probe profiles; the global probe default remains off. Plans 38–46 and previous Unreleased runtime/Windows improvements are included in this release; future changes will be listed under **Unreleased**.
...
| Windows Bash runtime — **0.33.0** | `auto`, `native-bash`, `wsl` | `auto` prefers Git for Windows and never silently selects WSL; explicit WSL uses `wsl.exe --exec bash -lc`. Optional Bash/Git executable overrides keep the toolchain association explicit. |
...
| Remote mutation guard — **0.33.0** | `--allow-remote-mutations` | Standard `git` / `gh` remote mutations are blocked and inherited GitHub tokens removed by default; opt in only for an explicitly authorized remote side effect. |
| Interruption recovery — **0.33.0** | receipt + `wait_for_handoff` | Records parent/child PIDs, distinguishes `interrupting`, `interrupted`, and stale `orphaned` runs, and requires reconciliation before retrying ambiguous material side effects. |
