...
When task continuation is enabled, arm continuation only for a substantial task likely to span model/MCP calls. After material progress call `continuation_checkpoint`; actual long work remains in `proc_*`, `job_*`, `goal_*`, or `batch_*` state. Before a truthful semantic yield with recorded work remaining, call `continuation_request`. Dispatch is user-gated: never auto-send, never wait/busy-poll just to trigger another turn, and never lower quality to fit a deadline. Call `continuation_complete` only after acceptance criteria and required verification are complete, or `continuation_cancel` when the user explicitly stops/cancels the overall task.
