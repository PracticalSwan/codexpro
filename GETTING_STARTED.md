...
Stable **0.33.2** includes all completed Plans 38–46, the post-0.32.4 Windows Bash and handoff maintenance, and current tunnel/HTTP continuity fixes. Build current main only when future verified **Unreleased** changes are required:
...
For most users, run **one CodexPro runtime** and allow every project you want that runtime to serve. Stable 0.33.2 includes the current multi-session/isolation fixes. If later changes are listed under **Unreleased** in `CHANGELOG.md`, use current `main` only when you specifically need those newer changes.
