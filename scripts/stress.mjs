...
async function runNodeFallbackSearchLimitStress() {
  ...
  try {
    ...
    assert(backend.structuredContent.searchBackend?.capabilities?.regex === true && backend.structuredContent.searchBackend?.regexEngine === 'javascript-worker',
      `node search backend did not advertise bounded regex capability: ${JSON.stringify(backend.structuredContent.searchBackend)}`);
  ...
  try {
    ...
    const finite = await client.request('tools/call', {
      ...
      arguments: {
        ...
        command: `${JSON.stringify(process.execPath)} -e "process.stdout.write('x'.repeat(8000))"`,
        timeout_ms: 15000
      }
    });
    ...
    assert(retainedBytes < 9000, `runaway-output bash retained too much output: ${retainedBytes} bytes`);
  } finally {
    client.close();
