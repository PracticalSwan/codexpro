...
稳定发布版是 **0.33.1**；当前 main 属于 0.33.1 发布线。0.33.1 修复 Artifact Export 的工具名称诊断错误，并支持通过 `codexpro settings user-default --local-service-probe on` 为当前操作系统用户的未来配置启用本地服务探测；其他用户仍默认关闭。此版包含 Plans 38–46 的运行时溯源、诊断、代码上下文、只读 notebook/表格/依赖检查及默认关闭的本机 HTTP 检查，并保留 0.32.4 的 Tunnel 和 Windows 稳定性修复。
...
上面的 tarball 是稳定版 0.33.1，包含 Plans 38–46 与 Tunnel heartbeat / child recovery。只有未来出现 Unreleased 更新时，才从 main 构建。
