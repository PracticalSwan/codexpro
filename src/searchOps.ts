...
export interface SearchBackendInfo {
  ...
  capabilities: { literal: true; regex: true };
  regexEngine: "ripgrep" | "javascript-worker";
  reason: string;
}
...
export function searchBackendInfo(): SearchBackendInfo {
  ...
  if (!ripgrepPath) {
    return {
      ...
      ripgrepVersion: null,
      capabilities: { literal: true, regex: true },
      regexEngine: "javascript-worker",
      reason: "ripgrep unavailable; literal search uses the Node fallback and regex search uses an interruptible Node worker"
    };
  }
