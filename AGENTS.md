...
- For Plans 29–37 continuation work, ChatGPT/provider authentication is a mandatory human boundary: open only the dedicated managed browser profile, STOP and report `WAITING_FOR_USER_AUTH`, never request/capture credentials or security codes, and resume only after the user authenticates directly in the browser and sends `continue`. Telegram setup is also human-gated: STOP for `WAITING_FOR_TELEGRAM_BOT_TOKEN` and `WAITING_FOR_TELEGRAM_PAIR`; the user creates/configures the bot and token locally and sends `continue`. Never ask for the Telegram token in chat. Version 1 dispatch remains explicitly user-authorized (browser or paired Telegram button) and must not auto-submit or scrape ChatGPT output. Watchdog timing must use the current runtime deadline/generation/transport snapshot (never the saved/default deadline); completed/canceled/user-paused tasks, stopped transport, manual user turns/Stop actions, and ambiguous/busy/error ChatGPT UI suppress continuation and must never trigger auto-Retry/model switching/tunnel restart.
