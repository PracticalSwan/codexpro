...
## Unreleased
- Simplified the future roadmap around consolidation and operator reliability: added detailed Plans 38–44, retired speculative MCP approval/Tasks work, froze Docker at its verified Windows Stage A scope, reduced durable project memory to current facts, and removed completed roadmap-controller/local packaging residue.
...
## 0.32.4 (2026-09-14)
- Refreshed GitHub-facing documentation and Pages for 0.32.4: promote long-lived OpenAI tunnel recovery into the stable release documentation, update ChatGPT Apps/Developer-mode setup wording to current OpenAI guidance, clarify the launcher tunnel-recovery security boundary, and add canonical/social metadata plus crawl files for the English/Chinese Pages site.
- Fixed long-lived OpenAI Secure MCP Tunnel tool-call loss: CodexPro now sets tunnel-client's MCP response-forwarding TTL to the 60-minute CodexPro bounded-call ceiling plus a five-minute transport margin instead of inheriting tunnel-client's 10-minute default; the launcher also heartbeats `/readyz`, marks runtime transport unavailable during persistent tunnel failure, and restarts only the tunnel-client child with bounded backoff while preserving the local MCP server, runtime generation, workspace selection, and tunnel lease; the Windows fake tunnel regression fixture also exits when its launcher health state is removed so smoke runs do not leave orphaned Node processes.
- Fixed Windows/Node 24 CI races in HTTP-state continuity: transient temporary-directory locks now get bounded `fs.rm` retries after child teardown; structured-job startup uses one parent-side OS start-identity attestation plus nonce/PID/start-key worker ownership proof instead of a redundant worker-side PowerShell lookup; read-only reconciliation keeps a live, matching owner-attested worker running when a fresh OS identity probe is temporarily unavailable, while concrete mismatches/dead PIDs still interrupt and cancellation still requires a fresh matching OS identity before signaling; MCP durability/continuity smokes, admin fixtures, and OpenAI tunnel lease assertions explicitly reuse/compare canonical workspace identities instead of raw Windows temp aliases; smoke timeouts now identify the exact tool and give only the composite `codexpro_self_test` a 60-second budget while ordinary calls remain at 15 seconds; the job-runner regression is now part of `npm run smoke`; and transitive `hono` is refreshed to 4.13.7 to clear the current moderate security advisories.
- Clarified deadline/resumability behavior: the synchronous deadline is per MCP call rather than a whole-ChatGPT-turn timer, durable resumable state should be persisted before risky long phases, and operators should leave a multi-minute margin below an observed host cutoff.
